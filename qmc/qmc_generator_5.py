...
class QMC_gen_v4():
    ...
    def metropolis_sampling(self, n_samples, burn_in, start=None, n=1, l=0, m=0):
        if start is None:
            # Initialize random starting position in 3D (x, y, z)
        ...
        psi_real, psi_imag = self.wavefunction(current_pos, n, l, m)
        ...
        for i in range(total_steps):
            ...
            try:
                psi_real_prop, psi_imag_prop = self.wavefunction(proposal, n, l, m)
                wf_proposal_mag_sq = psi_real_prop**2 + psi_imag_prop**2
            except Exception as e:
                print(f"Wavefunction error at position {proposal}: {e}")
