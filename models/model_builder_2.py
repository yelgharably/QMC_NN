...
from .base_network_5 import WavefunctionNN
from ..training.trainer_3 import load_model
import torch
import numpy as np
...
model_name = "model_049_tanh.pth"

def target_function(inputs, *params):
    x, y = inputs
    N = 3  # Degree of power series
    M = 3  # Number of Fourier terms
...
def load_model(model, model_name):
    ...
    # Use map_location to ensure it loads on CPU-only machine
...
with torch.no_grad():
    psi_values = model(inputs).cpu().numpy()
...
plt.savefig(os.path.join(output_dir, f'fitted_function_{timestamp}.png'))
...
print(f"Fitted function parameters saved to 'fitted_function_params_{timestamp}.json'.")
